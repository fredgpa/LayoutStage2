function [ population, problem, stats, evaluated] = evaluate_population(population, problem, settings, stats, start_time, f, best_exists, evaluated)
...
if (best_exists)||settings.reinitialize
    ...
    for i=settings.n_ind+1:settings.n_ind*2
      temp = [];
        [temp.fx, population.ind{i}, problem] = f.evaluate_individual(population.ind{i}, problem, population.fx(i,:));
        population.fx(i,:) = temp.fx;
        temp.ind = population.ind{i};
        evaluated = [evaluated temp];
    end
...
if (best_exists)
    ...
    if (length(problem.minimization)==1)
        ...
        if (problem.minimization)
            ...
            if (population.best_fx>best_fx)% && (alignment_test(problem, best))
