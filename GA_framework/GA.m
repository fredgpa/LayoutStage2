function [ population, stats, settings, problem ] = GA( problem, settings, population )
  ...
  evaluated = [];
  [population, problem, stats, evaluated]= f.evaluate_population(population, problem, settings, stats, stats.start_time, f, 0, evaluated);
  ...
  while (~settings.halt)
    ...
    %%disp(['Individuos reproduzidos: ', num2str(index2)])

    % Evaluation of the new population
    [population, problem, stats, evaluated] = f.evaluate_population(population, problem, settings, stats, stats.start_time, f, 1, evaluated);
    ...
    if (~strcmp(settings.adaptation,'off'))
      population = f.adaptation(population, problem, settings);
    end
    ...
    if ~cross
      f.crossover = str2func([problem.name,'_crossover_',problem.crossover_method2]);
      cross = true;
    else
      f.crossover = str2func([problem.name,'_crossover_',problem.crossover_method1]);
      cross = false;
    end

    index = index+1;
      if (population.best_fx - fx_backup) <= 0.01*population.best_fx && fx_backup ~= 0
          settings.halt = true;
      end
      fx_backup = population.best_fx;
  end
